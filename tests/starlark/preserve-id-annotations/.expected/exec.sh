...
kpt fn source --fn-config fn-config.yaml | docker run -i ghcr.io/kptdev/krm-functions-catalog/starlark:unstable | docker run -i ghcr.io/kptdev/krm-functions-catalog/format:v0.1 | kpt fn sink new-dir
mv new-dir/* .
rm -rf new-dir
